...
class Student extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('st_id, cent_id', 'required'),
			array('cent_id', 'numerical', 'integerOnly'=>true),
			array('st_id', 'length', 'max'=>32),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('st_id, cent_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'classes' => array(self::MANY_MANY, 'Class', 'classentrolled(st_id, cl_id)'),
			'cent' => array(self::BELONGS_TO, 'Center', 'cent_id'),
			'st' => array(self::BELONGS_TO, 'Person', 'st_id'),
		);
	}
